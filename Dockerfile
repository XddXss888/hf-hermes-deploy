...
RUN apt-get update && apt-get install -y \
    ...
    libasound2 libx11-xcb1 libfontconfig1 libx11-6 libxcb1 \
    libxext6 libxshmfence1 libglib2.0-0 libgtk-3-0 \
    libpangocairo-1.0-0 libcairo-gobject2 libgdk-pixbuf-2.0-0 \
    libxss1 libxtst6 libatspi2.0-0 \
    fonts-liberation fonts-noto-color-emoji fonts-wqy-zenhei fonts-freefont-ttf \
...
RUN pip install --no-cache-dir -r /tmp/requirements.txt && pip install --no-cache-dir 'anthropic>=0.39.0'
...
# Playwright + agent-browser + CloakBrowser
...
RUN pip install --no-cache-dir cloakbrowser && python -c "from cloakbrowser.download import ensure_binary; print(ensure_binary())" && chmod -R a+rX /opt/cloakbrowser
