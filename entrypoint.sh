...
declare -A PROVIDER_MODELS=(
    ...
    ["openai"]="gpt-4o"
    ["custom"]="Kimi-K2.6-UD-Q4_K_XL-00001-of-00014.gguf"
...
declare -A PROVIDER_BASE_URLS=(
    ...
    ["custom"]="${OPENAI_BASE_URL:-http://127.0.0.1:8080/v1}"
...
detect_main_model() {
    ...
    for provider in nvidia siliconflow custom openai anthropic google openrouter longcat; do
...
tools:
  platform_toolsets:
    api_server: [web, terminal, file, browser, skills, todo]
...
if [ -z "$AUTH_TOKEN" ]; then
    ...
    echo "   ✅ 使用环境变量中的 AUTH_TOKEN，并已覆盖持久化 Web UI Token"
fi
export AUTH_TOKEN
